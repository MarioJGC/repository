...
9.  CUDA (Arquitectura de dispositivo unificado de cómputo)
...
-   La GPU (Graphics Processing Unit) está optimizada para computación
    ...
    a.  **Por qué es importante en el aprendizaje automático**
...
b.  **Relación con GPT-2 o Transformadores**
...
2\. ¿Por qué usar una GPU (CUDA) acelera el entrenamiento?
...
3\. ¿Qué sucede internamente cuando llamas a
...
4\. ¿Por qué podemos usar GPT-2 sin entrenarlo nosotros mismos?
...
Corre el modelo GPT-2 CLI de ejemplo y responde:

1\. Son los pesos o weights descargados siempre? Por que o Por que no?
...
2\. Que cambios hay al pasar de gpt2 a distilgpt2?
...
3\. El script realiza entrenamiento o inferencia?
...
4\. ¿Por qué la generación tarda más en la CPU en comparación con la
GPU?
...
Ejercicio: Comparación GPT-2 y Qwen3 1.7B
...
Conecta tus hallazgos experimentales con estos conceptos. Usa ejemplos
específicos de tus resultados como evidencia.
...
• Que es instruction tuning? El ajuste de instrucciones es el proceso de
...
• Que es alignment? Alineación es el paso en el que se le enseñan al
modelo las preferencias humanas - específicamente a ser útil y
estructurado

• ¿Cómo el fine-tuning cambia el comportamiento del modelo sin modificar
su arquitectura? Dado que el fine-tuning permite obtener resultados
...
• ¿Pueden dos modelos de arquitectura similar comportarse de forma
diferente? Por que?
